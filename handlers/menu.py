...
from utils import answer_rich, edit_rich, send_photo_rich, send_rich, message_entities_from_dicts, telegram_utf16_length, _repair_custom_emoji_entities
from text_catalog import text as t
import crypto
from subscription import is_config_expired, enrich_configs_with_subscription_names
from utils import show_menu_with_sticker, get_main_keyboard
from states import UserStates, AdminStates
import bot_info
from config import ADMIN_ID, DATABASE_PATH, REFERRAL_LOCK_AMOUNT
...
def _user_guide_entities(guide: dict) -> list:
    """Entityهای عنوان و بدنه را برای متن نهایی راهنما حفظ می‌کند."""
    entities = []
    for entity in message_entities_from_dicts(guide.get("title_entities") or []):
        entities.append(entity)

    body_entities = message_entities_from_dicts(guide.get("body_entities") or [])
    if body_entities and guide.get("body_text"):
        prefix_len = telegram_utf16_length(f"{guide.get('title', '')}\n\n")
...
async def menu_user_guide_open(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Premium/Custom Emoji یا entity قدیمی نباید مانع باز شدن راهنما شود.
