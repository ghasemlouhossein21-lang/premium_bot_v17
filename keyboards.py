...
def _button_premium_emoji_kwargs(kwargs):
    ...
    if text:
        try:
            ...
            # Guide titles have an explicit text:<title> mapping.
            # Fall back to that exact key so trailing custom emojis are also
            # converted to Telegram's official button icon field.
            if not emoji_id:
                emoji_id = db.get_button_custom_emoji_id("text:" + str(text))
                if emoji_id:
...
def my_configs_list_keyboard(configs, icon: str, back_callback: str):
    ...
    for cfg in configs:
        ...
        buttons.append([InlineKeyboardButton(
            text=f"{icon} {display_name}", callback_data=f"viewconfig_{cfg['id']}", style="primary"
        )])
...
def admin_panel_menu(orders_enabled: bool = True, permissions: set[str] | None = None, is_main_admin: bool = True):
    ...
    add("👥 کاربران بدون خرید", "admin_userlist", "users")
    add("🔍 جستجوی حرفه‌ای", "admin_search", "users")
    add("🔎 جستجوی کانفیگ", "admin_config_search", "users")
...
def admin_services_list_keyboard(configs, uid: str):
    ...
    for cfg in configs:
        ...
        display_name = cfg.get("_display_name") or cfg.get("plan") or "سرویس"
        buttons.append([InlineKeyboardButton(
            text=f"{mark}{icon} {display_name}", callback_data=f"svcdetail_{cfg['id']}", style="primary"
        )])
