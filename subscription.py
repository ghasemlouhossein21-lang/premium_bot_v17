...
def _decode_profile_title(headers: dict) -> str | None:
    """Decode the exact service name advertised by the subscription endpoint.

    Panels are inconsistent here: some return plain UTF-8, some return standard
    base64, some URL-safe base64, and some URL-percent-encode the header first.
    Never normalize the decoded value because the caller needs the exact panel
    name (including emoji, punctuation and spacing).
    """
    raw = headers.get("Profile-Title") or headers.get("profile-title")
    if not raw:
        return None
    raw = unquote(str(raw)).strip().strip('"')
    if raw.lower().startswith("base64:"):
        raw = raw[7:].strip()

    # First try it as a normal title. If it looks like base64, also try both
    ...
    for value in candidates:
        if not value:
            continue
        # Reject binary/control garbage that can result from accidentally
        # decoding a normal plain-text title as base64.
...
async def get_subscription_display_name(sub_url: str) -> str | None:
    ...
    try:
        ...
        name = name.strip()
        if not name or name.casefold() in {"subscription", "profile", "sub", "default"}:
            return None
        return name
    except Exception:
        return None
...
async def enrich_configs_with_subscription_names(configs: list[dict]) -> list[dict]:
    """برای لیست سرویس‌ها نام نمایشی را از لینک ساب پر می‌کند.

    اگر لینک ساب قابل دریافت نباشد، نام ذخیره‌شده‌ی پلن بدون تغییر باقی می‌ماند.
    """
